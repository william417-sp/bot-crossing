...
export function createGarage(scene: THREE.Scene, sites: Site[]): GarageElements {
  ...
  return { floor, walls, ceiling, holograms, workstations };
}
...
function createWalls(): THREE.Group {
  ...
  for (let i = 0; i < panelCount; i++) {
    ...
    const frameMat = new THREE.LineBasicMaterial({ color: 0x3c5a8c, transparent: true, opacity: 0.4 });
...
function createToolRacks(walls: THREE.Group): void {
  ...
  for (const xPos of rackPositions) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const toolMat = new THREE.MeshStandardMaterial({ color: 0x3a4050 });
...
function createVentGrilles(walls: THREE.Group): void {
  for (let i = 0; i < 4; i++) {
    ...
    const ledMat = new THREE.MeshBasicMaterial({ color: 0x3cb4ff });
...
function createCeiling(): THREE.Group {
  ...
  for (const xPos of lightPositions) {
    ...
    const fixtureMat = new THREE.MeshStandardMaterial({ color: 0x12151e });
    const fixture = new THREE.Mesh(fixtureGeo, fixtureMat);
    fixture.position.set(xPos, GARAGE_HEIGHT - 0.08, 0);
    ceiling.add(fixture);
    
    const ledStripGeo = new THREE.BoxGeometry(1.0, 0.05, 0.25);
    const ledStripMat = new THREE.MeshBasicMaterial({ color: 0xe8f0ff });
...
function createFloatingHolograms(): THREE.Group {
  ...
  for (const pos of holoPositions) {
    ...
    if (pos.type === 'cube') {
      ...
      const edgesMat = new THREE.LineBasicMaterial({ color: 0x50dcff, transparent: true, opacity: 0.8 });
      ...
      const wireMat = new THREE.LineBasicMaterial({ color: 0x50dcff, transparent: true, opacity: 0.6 });
      ...
      const ringMat = new THREE.MeshBasicMaterial({ color: 0x64dcff, transparent: true, opacity: 0.5 });
      ...
      const frameGeo = new THREE.EdgesGeometry(panelGeo);
      const frameMat = new THREE.LineBasicMaterial({ color: 0x3cdcff, transparent: true, opacity: 0.7 });
      const frame = new THREE.LineSegments(frameGeo, frameMat);
      holoGroup.add(frame);
      
      for (let i = 0; i < 4; i++) {
        const lineGeo = new THREE.PlaneGeometry(0.3 + Math.random() * 0.2, 0.02);
        const lineMat = new THREE.MeshBasicMaterial({ color: 0x64f0ff, transparent: true, opacity: 0.5 });
...
function createWorkbench(group: THREE.Group): void {
  const tableMat = new THREE.MeshStandardMaterial({ color: 0x2a3444, roughness: 0.6, metalness: 0.3 });
  ...
  const legMat = new THREE.MeshStandardMaterial({ color: 0x1a222e });
  ...
  const toolMat = new THREE.MeshStandardMaterial({ color: 0x4a5568 });
...
function createHolodesk(group: THREE.Group): void {
  const baseMat = new THREE.MeshStandardMaterial({ color: 0x1a222e, roughness: 0.5, metalness: 0.4 });
  ...
  const ringMat = new THREE.MeshBasicMaterial({ color: 0x3cc8ff, transparent: true, opacity: 0.4 });
  const ring = new THREE.Mesh(ringGeo, ringMat);
  ring.rotation.x = -Math.PI / 2;
  ...
  const wireGeo = new THREE.WireframeGeometry(sphereGeo);
  const wireMat = new THREE.LineBasicMaterial({ color: 0x64dcff, transparent: true, opacity: 0.6 });
  const sphere = new THREE.LineSegments(wireGeo, wireMat);
...
function createServerRack(group: THREE.Group): void {
  const rackMat = new THREE.MeshStandardMaterial({ color: 0x1a222e, roughness: 0.7, metalness: 0.3 });
  ...
  const unitMat = new THREE.MeshStandardMaterial({ color: 0x252d3a, roughness: 0.5, metalness: 0.4 });
  for (let i = 0; i < 4; i++) {
    ...
    const led1Mat = new THREE.MeshBasicMaterial({ color: 0x34d399 });
    ...
    const led2Mat = new THREE.MeshBasicMaterial({ color: 0x3b82f6 });
...
function createCharger(group: THREE.Group): void {
  const padMat = new THREE.MeshStandardMaterial({ color: 0x1a222e, roughness: 0.4, metalness: 0.5 });
...
function createFabricator(group: THREE.Group): void {
  const bodyMat = new THREE.MeshStandardMaterial({ color: 0x1a222e, roughness: 0.6, metalness: 0.4 });
  ...
  const frameMat = new THREE.LineBasicMaterial({ color: 0x506080 });
  const frameGeo = new THREE.EdgesGeometry(windowGeo);
  const frame = new THREE.LineSegments(frameGeo, frameMat);
  frame.position.copy(windowMesh.position);
  group.add(frame);
  
  const panelMat = new THREE.MeshStandardMaterial({ color: 0x252d3a });
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const ledMat = new THREE.MeshBasicMaterial({ color: 0x1e3a5a });
    const led = new THREE.Mesh(ledGeo, ledMat);
...
export function updateGarage(elements: GarageElements, time: number, sites: Site[]): void {
  ...
  elements.ceiling.traverse((obj) => {
    if (obj.userData.isCeilingLight) {
      const flicker = 0.92 + 0.08 * Math.sin(time * 1.8 + obj.position.x);
      (obj as THREE.Mesh).material = new THREE.MeshBasicMaterial({
        color: new THREE.Color(0xe8f0ff).multiplyScalar(flicker),
      });
    }
    if (obj.userData.isLightCone) {
      const pulse = 0.03 + 0.01 * Math.sin(time * 1.5 + obj.position.x);
